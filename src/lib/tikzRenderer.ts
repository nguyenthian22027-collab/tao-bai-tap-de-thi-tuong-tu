...
export function normalizeTikzCode(code: string): string {
  ...
  clean = clean.replace(/[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵđÀÁẢÃẠĂẰẮẲẴẶÂẦẤẨẪẬÈÉẺẼẸÊỀẾỂỄỆÌÍỈĨỊÒÓỎÕỌÔỒỐỔỖỘƠỜỚỞỠỢÙÚỦŨỤƯỪỨỬỮỰỲÝỶỸỴĐ]/g, (c) => vnMap[c] || c);
...
\\usetikzlibrary{shapes,arrows,calc,intersections,patterns,decorations.pathreplacing,decorations.pathmorphing,decorations.markings,angles,quotes,arrows.meta,positioning,through,backgrounds,fit,matrix,chains,3d}
\\begin{document}
${normalized}
\\end{document}`;
}
